...
while True:
	...
	mouse_pos = (mouse_pos[0]*DRAWSIZE[0]/DISPLAYSIZE[0],mouse_pos[1]*DRAWSIZE[1]/DISPLAYSIZE[1])
	SCREEN.fill(DARK)
	
	if pygame.mouse.get_pressed()[0]:
		if mouse_pos[1] < 4:
			CURRENTCOLOR =  mouse_pos[0]//2
		else:
			pygame.draw.line(DRAWSURF,COLORS[CURRENTCOLOR],mouse_pos,LASTMOUSEPOS,PINSELSIZE)
			pygame.draw.circle(DRAWSURF,COLORS[CURRENTCOLOR],mouse_pos,PINSELSIZE//2)
	FLAG = True
	while FLAG:
		FLAG = False
	...
	if pygame.mouse.get_pressed()[2]:
		DRAWSURF.set_at(mouse_pos,TRANSPARENT)
	UISURF.fill((0,0,0,0))
